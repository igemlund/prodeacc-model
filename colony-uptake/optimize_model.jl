...
t = 0.0:1:5.0
k_data = [V_max*5, k_m]
init = ions_in_cytoplasm(t, k_init, 0.25)
data1 = ions_in_cytoplasm(t, k_data, 0.25)
data2 = ions_in_cytoplasm(t, k_data, 0.50)
data3 = ions_in_cytoplasm(t, k_data, 0.125)

# Cost function for optimizing model, using different concentrations
f(x) = (loss_function(data1, ions_in_cytoplasm(t,x, 0.25))
    + loss_function(data2, ions_in_cytoplasm(t,x,0.5))
    + loss_function(data3, ions_in_cytoplasm(t,x,0.125)))

# Optimize
res = optimize(f, k_init, NelderMead())
